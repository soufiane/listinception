...
/*
 * Permissions
 * create a collection to manage the permissions
 */
...
var Permission = Backbone.Model.extend({
  ...
  validate: function (attributes) {
    if (this.perms.indexOf(attributes.permission) === -1) {
      return [{message: 'permission is unknown'}];
    }
  }*/
...
/*
 * Make sure there is only version of the model with the same id
 */
...
var List = Backbone.Model.extend({
  ...
  initialize: function() {
    //var childs = this.get('children');
    //this.children = new Lists(childs)
    //this.unset('children');

    // add the permission collection
    /*
    this.permissions      = new Permissions();
    this.permissions.url  = this.url() + '/permissions';

        perms = this.get('permissions');
    
    for (var key in perms) {
      self.permissions.add({id: key, permissions: perms[key]}, {silent: true});
    }
    */

    // how to add a new perm
    // create a new perm model
    // save to /posts/id/perms
